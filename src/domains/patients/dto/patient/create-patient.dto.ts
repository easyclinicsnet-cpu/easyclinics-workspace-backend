...
/**
 * Create Patient DTO
 * Used for creating new patient records with full validation and multi-tenancy support
 *
 * Multi-tenancy: workspaceId is required for tenant isolation
 * Encryption: firstName, lastName, gender, birthDate, phoneNumber, email, city, address, nationalId are encrypted
 * Insurance: Optional insurance fields with conditional validation
 */
export class CreatePatientDto {
  ...
  @ApiPropertyOptional({
    description: 'Workspace ID for multi-tenant isolation (injected from JWT, do not send in body)',
  ...
  // ===== REQUIRED PATIENT DEMOGRAPHICS (ENCRYPTED) =====
  @ApiProperty({
  ...
  // ===== OPTIONAL CONTACT INFORMATION (ENCRYPTED) =====
  @ApiPropertyOptional({
  ...
  @ApiPropertyOptional({
    description: 'Flag to create/update patient insurance record',
    example: true,
    default: false,
  })
  @IsOptional()
  ...
  // When the client sends `membershipNumber`, whitelist:true strips it from the body,
  // so we read from the raw request object via TransformFnParams.obj which still holds
  // the original values before stripping occurs. This keeps backward compatibility
  // without requiring the client to rename the field.
